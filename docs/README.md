...
| [`coverage-remediation-plan.md`](coverage-remediation-plan.md) | **A PLAN, not a change.** Eight items, one per session, from a fourteen-agent adversarial review on 2026-09-14. Measured: 7 of 146 documents publish any fact, 0 step reviews ever, 0 of 6,660 figures carry any text, 0 values from `data/structural/`. Items 1–3 are free and need no curator; item 4 is the leverage change; item 8 waits on Planning. §0a records what was measured and found **not** wrong, so it is not re-litigated |
