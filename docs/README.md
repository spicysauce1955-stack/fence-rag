...
| Document | What it is |
|---|---|
| [`level-1-publication-finding.md`](level-1-publication-finding.md) | **A MEASUREMENT that cancels a planned item**, 2026-09-14. The 524 `cross_family_verified` readings are one reader's copy of cells a person already accepted, in BOM columns no parameter builder matches — publishing them yields zero rows. Obligation 6 does not require it, and the obvious implementation would have recreated the defect CUR-S0 closed. **What replaces it: 177 human-reviewed parts-list rows that publish nothing while 42 `Part`s are published by hand-written Python** |
| [`keyword-ruler-audit.md`](keyword-ruler-audit.md) | **A MEASUREMENT and a triage**, 2026-09-14. Every retrieval figure this project published was measured on hand-written `query_terms` that production never sends. Four decisions re-measured: R3 stands (structurally, and better than the old column showed), R5 stands (its recorded figure does not reproduce), R1 is moot, the second stage is reversed and shipped. **The deeper finding: 15 of 78 gold questions leak an answer term into their own search terms, and the keyword column's entire support advantage is that leakage.** Repair the gold set before re-measuring anything else |
...
| [`coverage-remediation-plan.md`](coverage-remediation-plan.md) | **A PLAN, not a change.** Nine items on **three tracks, ordered by who can act** — engineer alone (0,1,2,3,5,6), engineer-then-curator (4,7), blocked on Planning (8). From a fourteen-agent adversarial review, re-verified 2026-09-14. Measured: 7 of 146 documents publish any fact, 0 step reviews ever, 0 of 6,660 figures carry any text, 0 values from `data/structural/`. §0a records what was measured and found **not** wrong; §0c is a re-measure step, because the store predates HEAD by one commit that touched the publishing path |
