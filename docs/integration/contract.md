...
> tie, resolution breaks it by higher `curation_level`, then later `issue_date`, then
...
6. **Every published value carries an honest `source_class` and `curation_level`** — not
   only every parameter row. A rail length has the same admissibility problem as a footing
   depth. Nothing reaches level 2 without a person having compared it to the source image,
   and resolution honours the source policy, recording `admitted_by` on the winner.
