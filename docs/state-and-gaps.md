...
| Structured facts (Phase 6) | `facts.py` | 1,652 facts, all provenance-bearing |
| Reports | `reports.py` | 6 generated reports, regenerated by `cli report` |
| Tests | `tests/` | 316 tests, stdlib only; all pass over a full store, `OK (skipped=7)` after a clean rebuild (the promotion tests) |
...
| elements: lang (en·assumed / und·unknown / zh) | 59,341 / 22,453 / **0** — A4, see G32 |
| schema_version | 2 |
| quality issues | 374 |
...
cell and nothing else. 266 of 1,652 facts are `flagged` because they came from
...
**59,341** `en`/`assumed` and **22,453** `und`/`unknown`.
...
conditions rather than inside them. Today: **117** `assumed`, **1,535**
`unexamined`, **0** `stated`.

**Three values, not the contract's two.** Obligation 15 names `stated` and
`assumed`. A third, `unexamined`, is internal and publishes as `assumed`: 1,535
