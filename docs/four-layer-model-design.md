...
| Human review decisions to preserve | **3 table reviews, 144 stamped readings, 24 promoted facts** — 2026-08-30. Was 0 when this table was written. |

So the migration is a **rebuild, not an in-place data migration**: no risky
`ALTER`, no backfill, no id remapping.

**The window this section called "the cheapest it will ever be" has closed.** It said
*"the first accepted reading makes the migration expensive"*, and on 2026-08-30 a person
accepted three crops. The cost is smaller than that sentence feared, because G49 landed
first: human decisions are no longer irreplaceable store state. `review-ledger.jsonl` is
committed, keyed on evidence rather than on any row id — `crop_sha256` for a table review
— and `[measured]` that day it replays into a store with the reviews dropped and
reproduces all 24 facts and all four `ParameterTable`s. A rebuild under the four-layer
model re-imports the ledger; it does not have to preserve `table_reviews` rows.

What is *not* cost-free any more is the `from_candidates` residual G49 already names:
those are `table_read_candidates` rowids, and a reload in a different order moves them.
Three reviews' worth is a small thing to repair; 44 crops' worth would not be.
